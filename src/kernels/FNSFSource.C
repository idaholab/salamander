...
FNSFSource::validParams()
{
  ...
  params.addRequiredParam<std::vector<Real>>(
      ...
      "outboard blanket.");
  params.addParam<Real>("coeff", 1.0, "coeff for source");
  return params;
}

FNSFSource::FNSFSource(const InputParameters & parameters)
  : Kernel(parameters),
