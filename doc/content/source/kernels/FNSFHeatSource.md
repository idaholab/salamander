...
in through the `heat` parameter; this kernel maps each quadrature point's physical location to a
cell on that grid and applies the corresponding value as a `Kernel` residual contribution:
...
then located on the grids given by `inner_xi`, `outer_xi`, and `depth` (which describe the inner
first-wall edge, outer back-wall edge, and radial depth divisions of the neutronics grid,
respectively) to select the correct entry of `heat`. This grid-projection approach is detailed in
