...
`BlanketProblem` is a specialized [FEProblem.md] used to couple a 1D Thermal Hydraulics Module
...
corresponds to. `BlanketProblem` provides this association: setting its
[!param](/Problem/BlanketProblem/master_bdry_name) parameter on a sub-application (typically via
`cli_args` on the `MultiApps` block) records the
...
[!param](/Problem/BlanketProblem/master_bdry_name):
