...
class BorisCyclotronMagneticTest(unittest.TestCase):
    """
    This test case tests the Boris stepper using the case of cyclotron motion.
    In this test a single particle is subject to a static magnetic field and the
    solution for the particle's path is known analytically.
...
class BorisTimeVarryingElectricTest(unittest.TestCase):
    """
    This test case tests the Boris stepper using a time-varying electric field that
    is spatially uniform.
    The exact form of the electric field is calculated by propagating the solution for
    the particle's path that is provided by the cyclotron motion case through Newton's
