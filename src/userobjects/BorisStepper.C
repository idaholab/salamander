...
registerMooseObject("SalamanderApp", BorisStepper);
    ...
    _bfield_vars(getParam<std::vector<VariableName>>("bfield_components"))
{
  ...
  for (int i = 0; i < 3; ++i)
  {
    _efield_samplers.push_back(SALAMANDER::VariableSampler(_fe_problem, _efield_vars[i], _tid));
    _bfield_samplers.push_back(SALAMANDER::VariableSampler(_fe_problem, _bfield_vars[i], _tid));
  }
}
